...
class User(db.Model):  # Создание модели для таблицы User в базе данных
    ...
    def __init__(self, name, gender, age, country, sport, active):  # Метод для инициализации экземпляра класса User
...
with app.app_context():  # Вход в контекст приложения
    ...
    user1 = User('Jo', 'F', 30, 'USA', 'BJJ', 1)  # Создание нового пользователя
    user2 = User('Eliza', 'M', 18, 'USA', 'Swimming', 0)  # Создание нового пользователя
    ...
    for user in users: #вывод пользователей
        print(f"User ID: {user.id}, Name: {user.name}, Gender: {user.gender}, Age: {user.age}, Country: {user.country}, Sport: {user.sport}, Active: {user.active}")
...
@app.route('/contacts/')  # Определение маршрута для URL /contacts.html
